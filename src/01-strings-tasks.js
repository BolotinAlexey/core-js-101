 ...
 *   'cat', 3 => 'catcatcat'
 */
function repeatString(s, c) {
  return new Array(c).fill(s).join('');
  // throw new Error('Not implemented');
}
 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(s, rem) {
  return s.replace(rem, '');
  // throw new Error('Not implemented');
}
...
function getRectangleString(w, h) {
  let s = new Array(w).fill('─').map((el, i) => {
    if (!i) return '┌';
    return i === w - 1 ? '┐\n' : el;
  });
...
function encodeToRot13(s) {
  ...
  source = source.split('');
  return s.split('').map((ch) => { const ind = source.indexOf(ch); return ind === -1 ? ch : source[ind % 26 >= 13 ? (ind + 39) % 52 : ind + 13]; }).join('');
  // throw new Error('Not implemented');
}
